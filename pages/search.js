...
export default function Search({ /*data*/ }) {
  ...
  const [notFound, setNotFound] = useState();
   const [inputValue, setInputValue] = useState(/*username*/"borra esto para buscar" || keyword || " ");
  ...
  return (
    ...
    <Navbar />
      ...
      />
      <Page>
        <h1 className="text-4xl mb-4 font-bold">Busca los profesionales de Pinamar🌲</h1>
        ...
        <div className="relative">
          <input
            ...
            ref={inputRef}
            className="border-2 hover:border-blue-600 transition-all duration-250 ease-linear rounded px-6 py-2 mb-4 block w-full"
            name="keyword"
            value={inputValue}
            onChange={(e) => setInputValue(e.target.value)}
          />
          {filteredUsers && (
            <div className="absolute inline-block top-0 right-0 bottom-auto left-auto translate-x-1/4 -translate-y-1/3 rotate-0 skew-x-0 skew-y-0 scale-x-100 scale-y-100 py-1 px-1.5 text-xs leading-none text-center whitespace-nowrap align-baseline font-bold bg-blue-600 text-black rounded-full z-10">
